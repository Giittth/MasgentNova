...
# ===================================================================
# ⚠️  DEPRECATED: This file has been split into masgent.tools/
#    Please do not modify this file. All changes should go to:
#    - masgent/tools/core.py
#    - masgent/tools/vasp.py
#    - masgent/tools/structure.py
#    - masgent/tools/mlp.py
#    - masgent/tools/ml.py
#    - masgent/tools/workflow.py
# ===================================================================
...
def rename_file(name: str, new_name: str) -> dict:
    ...
    try:
        ...
        # 使用 resolve() 解析符号链接和相对路径，再用 is_relative_to 判断
...
def generate_vasp_poscar(formula: str) -> dict:
    ...
    try:
        ...
        if not config.mp_api_key:
            ask_for_mp_api_key()  # 该函数会设置 os.environ 并写入 .env
            # 重新加载配置（或在 ask_for_mp_api_key 中直接设置 config.mp_api_key）
            # 但 pydantic-settings 不支持动态修改，所以更简单的方式是直接用 os.environ
...
def generate_vasp_poscar_with_interstitial_defects(
    ...
    try:
        from pymatgen.core import Structure  # 添加这一行
        runs_dir = str(config.get_runs_dir())
